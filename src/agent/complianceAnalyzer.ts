...
import type { AgentAnswer, QuestionPayload, SearchResult } from "./types";
...
export class ComplianceAnalyzer {
  ...
  async analyze(
    ...
    const response = await toolRunner.track(createStepId("llm-analyze"), async () => {
      ...
      contextResults: context.map((result) => ({
...
function buildPrompt(question: QuestionPayload, context: SearchResult[]): string {
  ...
  const contextText = context
    .map(
      (chunk, index) =>
        `\n[Fuente ${index + 1}]\nDocumento: ${chunk.documentId}\nTitulo: ${chunk.title ?? "(desconocido)"}\nReferencia: ${chunk.reference ?? "N/A"}\nTexto: ${chunk.snippet}`
    )
    .join("\n");
